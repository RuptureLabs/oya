...
from oya.db.migrations.ddl import BaseDDL
from oya.db.migrations.models import MAX_VERSION_LENGTH, Migration
from oya.db.migrations.utils import get_app_connection, get_models_describe, is_default_function
...
class Migrate:
    ...
    @classmethod
    def mkdir_migration_app(cls):
        if cls.migrate_location.exists():
            return
        
        cls.migrate_location.mkdir(parents=True, exist_ok=True)
    ...
    @classmethod
    async def get_last_version(cls) -> Optional[Migration]:
        try:
            return await Migration.filter(app=cls.app).first()
    ...
    async def load_ddl_class(cls):
        ddl_dialect_module = importlib.import_module(f"oya.db.migrations.ddl.{cls.dialect}")
        return getattr(ddl_dialect_module, f"{cls.dialect.capitalize()}DDL")

    @classmethod
    async def init(cls, config: dict, app: str, location: str):
        # ---- necessary for migrations per app -----
        cls.upgrade_operators = []
        cls.downgrade_operators = []
        cls._upgrade_fk_m2m_index_operators = []
        cls._downgrade_fk_m2m_index_operators = []
        cls._upgrade_m2m = []
        cls._downgrade_m2m = []
        cls._rename_old = []
        cls._rename_new = []
        ...
        cls.app = app
        cls.migrate_location = Path(location)
        if last_version:
    ...
    def diff_models(cls, old_models: Dict[str, dict], new_models: Dict[str, dict], upgrade=True):
        ...
        :return:
        """
        _migration = f"{cls.app}.{cls._migration}"
        
        old_models = old_models or {}
        new_models = new_models or {}

        old_models.pop(_migration, None)
        new_models.pop(_migration, None)
                    ...
                    for old_data_field in old_data_fields:
                        ...
                        if len(changes) == 2:
                            ...
                            if (
                                ...
                                if upgrade:
                                    print(
                                        f"rename {old_data_field_name} to {new_data_field_name} ? (y/n) default [n]"
                                    )
                                    ret = input().lower()
                                    is_rename = ret == "y" or ret == "yes"
                                else:
